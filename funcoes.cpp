...
bool verificaAlunos () {
	...
	int conta;
	std::ofstream arquivoDosAprovados;
	arquivoDosAprovados.open("listaAlunosAprovados.csv");

	if (!arquivoDosAprovados.is_open()) {
	...
	for (int i = 0; i < 100; i++) {
		if (alunos[i].media >= 6) {
			arquivoDosAprovados	<< alunos[i].nome << ";"
								<< alunos[i].matricula << ";"
								<< alunos[i].media << ";"
								<< alunos[i].status << std::endl;
		}
	}
	arquivoDosAprovados.close();
	...
	for (int i = 0; i < 100; i++) {
		if (alunos[i].media >= 6) {
			...
			std::cout << " Matricula: " << alunos[i].matricula << std::endl;
			conta++;
			}
	}

	if (conta == 0) {
		std::cout << "Nenhum aluno foi aprovado.";
	} else if (conta == 1) {
		std::cout << std::endl;
		std::cout << "Apenas um aluno foi aprovado" << std::endl << "Arquivo 'listaAprovados' atualizado!" << std::endl;
	} else {
		std::cout << std::endl << conta << " foram reprovados na disciuplina de banco de dados." << std::endl << "Arquivo 'listaAprovados' atualizado!" << std::endl;
	}
